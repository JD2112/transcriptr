####################################################
## TranscriptR Salmon single-end RNA-seq Pipeline ##
####################################################
...
# salmon index -p {threads} -t {input.fa} -i {output.idx} # without docker run
...
# fastqc {input.R1} -t {threads} -o {output.out} >> {log} 2>&1 # without docker run
