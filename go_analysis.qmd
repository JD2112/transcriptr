...
Here in *TranscriptR*, we used ontology analysis using the [clusterProfiler package](https://bioconductor.org/packages/release/bioc/html/clusterProfiler.html){target="_blank"} [@yu2012clusterprofiler; @wu2021clusterprofiler].
...
3. *Select P-value adjustment method* As per clusterProfiler, we set different *p*-value adjustment methods, Benjamini-Hochberg, Benjamini-Yeketuli, Bonferroni, Holm, Hommel, Hochberg, FDR or none. Default is Benjamini-Hochberg.
4. *Select ontology class*: As defined in GO classification, we included all three ontology classes which user can select to show the plot.

#### Data upload
At present, user can upload the DMC data produced by the main analysis (see @sec-transcript ) directly. The input file should be in a **text (tab-delimited)** format.
...
![Gene Ontology - Biological Processes](images/GO-BP.png)

### Cellular component
![Gene Ontology - Cellular Component](images/GO-CC.png)

### Molecular function
![Gene Ontology - Molecular Function](images/GO-MF.png)
...
![Gene Ontology data table](images/GO-table.png)
